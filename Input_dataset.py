...
def _generate_image_and_label_batch(image, label, min_queue_examples, batch_size, shuffle):
  ...
  # read 'batch_size' images + labels from the example queue.
    num_preprocess_threads = 1   # changed 1
    if shuffle:
...
#         image = tf.cast(tf.reshape(image_bytes, (IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNEL)), tf.float32)
#         label = tf.cast(tf.reshape(label_bytes, (IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNEL)), tf.float32)     
...
def CamVid_reader(filename_queue):
    ...
    image = tf.image.resize_images(image_bytes, [IMAGE_HEIGHT, IMAGE_WIDTH], method=0)
    label = tf.image.resize_images(label_bytes, [IMAGE_HEIGHT, IMAGE_WIDTH], method=0)
    ...
    label = tf.image.resize_image_with_crop_or_pad(label, IMAGE_HEIGHT, IMAGE_WIDTH)

#     image = tf.reshape(image_bytes, (IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNEL))
#     label = tf.reshape(label_bytes, (IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNEL))
...
def get_filename_list(path):
    '''
    CamVid
    Input - files which record the paths of raw images and point images
    Return - raw image_filenames & point_filenames
    '''
    fd = open(path)
    image_filenames = []
    point_filenames = []
    ...
    for i in fd:
        ...
        point_filenames.append(i[1])
    return image_filenames, point_filenames
...
#     image = CamVid_reader(filename_queue)
#     label = CamVid_reader_depth(filename_queue)
